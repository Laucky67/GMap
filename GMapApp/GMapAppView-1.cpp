...
void GMapAppView::DrawCache(int offsetX, int offsetY)
{
	...
	else
	{
		...
		//m_client_graphics->Clear(Color::White);
		m_client_graphics->DrawImage(m_offscreen_bitmap, offsetX, offsetY);
	}
}
...
void GMapAppView::PrepareMap()
{
	if (!m_cache_bitmap)
	{
		CRect rc;
		GetWindowRect(&rc);
		//RectF vp = m_map_drawing->GetViewPort();
		//rc.SetRect(vp.GetLeft(), vp.GetTop(), vp.GetRight(), vp.GetBottom());
